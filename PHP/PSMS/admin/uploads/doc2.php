...
if(isset($_POST['doc2_upload']))
{
	if(isset($_GET['id']))
	{
		...
		// File upload path
		$targetDir = '../images/'.$id.'/';
		$fileName = 'Doc2.' . pathinfo($_FILES['doc2']['name'],PATHINFO_EXTENSION);
		$targetFilePath = $targetDir . $fileName;
		$fileType = pathinfo($targetFilePath,PATHINFO_EXTENSION);
		$img_size=$_FILES['doc2']['size'];
		...
		if(!empty($_FILES["doc2"]["name"]))
		{
			...
			if($img_size<=1048576)
			{
				if(in_array($fileType, $allowTypes))
				{
					...
					if(move_uploaded_file($_FILES["doc2"]["tmp_name"], $targetFilePath))
					{
						// Insert image file name into database
						$sql = "UPDATE employee SET doc2=('".$fileName."') WHERE id = $id";
						if(mysqli_query($conn,$sql))
						{
							$_SESSION['msg']="Doc2 Has Been Uploaded Successfully.";
