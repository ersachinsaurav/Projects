...
class Category_model extends CI_Model {
	...
	public function getCategoriesArticles($params=[]){
		if(!empty($params['queryString'])){
			$this->db->like('name', $params['queryString']);
		}
	...
	public function statusUpdate($id, $categoryStatus){
		$status = array('status' => $categoryStatus);    
		$this->db->where('id', $id);
		$this->db->update('categories', $status);
   }
